...
const {
  checkReleaseVersion, releaseVersionFromTag, sourceVersion, stampSourceVersion, readSourceVersion,
...
test("release checks reject malformed or mismatched tags and versions", () => {
  ...
  assert.equal(releaseVersionFromTag("visualstudio-v1.2.3"), "1.2.3");
  for (const tag of ["", "v1.2.3", "visualstudio-v", "visualstudio-v01.2.3", null]) {
    assert.throws(() => releaseVersionFromTag(tag), /Release tag|VSIX version/);
  }
});
...
test("source validation uses Identity.Version and requires matching build metadata", () => {
  ...
  const packageSource = '[InstalledProductRegistration("Example", "Description", "1.2.3")]';
  assert.equal(sourceVersion(manifest, props, packageSource), "1.2.3");
  assert.equal(sourceVersion('<!-- <Identity Version="9.9.9" /> -->' + manifest, props, packageSource), "1.2.3");
  assert.equal(sourceVersion(manifest.replace('Version="1.2.3"', "Version='1.2.3'"), props, packageSource), "1.2.3");
  assert.throws(() => sourceVersion(manifest, props.replace("1.2.3", "1.2.4"), packageSource), /must match/);
  assert.throws(() => sourceVersion(manifest, props, packageSource.replace("1.2.3", "1.2.4")), /must match/);
  assert.throws(() => sourceVersion("<PackageManifest />", props, packageSource), /Expected one/);
  assert.throws(() => sourceVersion(manifest + manifest, props, packageSource), /Expected one/);
  assert.throws(() => sourceVersion(manifest, props + props, packageSource), /Expected one/);
  assert.throws(() => sourceVersion(manifest, props, packageSource + packageSource), /Expected one/);
});

test("release tags stamp every Visual Studio build version without changing other content", () => {
  const manifest = '<Identity Id="example" Version="0.1.0" Language="en-US" />';
  const props = '<Version>0.1.0</Version>\n<Authors>Example</Authors>';
  const packageSource = '[InstalledProductRegistration("Example", "Description", "0.1.0")]\npublic class Example {}';
  const stamped = stampSourceVersion(manifest, props, packageSource, "2.3.4");
  assert.match(stamped.manifest, /Version="2\.3\.4"/);
  assert.match(stamped.props, /<Version>2\.3\.4<\/Version>/);
  assert.match(stamped.packageSource, /"2\.3\.4"\)\]/);
  assert.match(stamped.props, /<Authors>Example<\/Authors>/);
  assert.match(stamped.packageSource, /public class Example/);
  assert.equal(sourceVersion(stamped.manifest, stamped.props, stamped.packageSource), "2.3.4");
});

test("release CLI stamps tagged builds and validates packaged versions against the tag", () => {
  ...
  for (const result of [run([]), run([], `visualstudio-v${version}`), run(["--stamp"], `visualstudio-v${version}`),
    run([version, `visualstudio-v${version}`])]) {
    assert.equal(result.status, 0, result.stderr);
    assert.match(result.stdout, /(Stamped|Verified).*Visual Studio version/);
  }
  const wrong = version === "9.9.9" ? "8.8.8" : "9.9.9";
  for (const result of [run([], `visualstudio-v${wrong}`), run([version]), run([wrong, `visualstudio-v${version}`]),
    run([""], `visualstudio-v${version}`), run([], "vscode-v1.2.3"), run(["--stamp"], "vscode-v1.2.3")]) {
...
test("workflow source restricts publishing to successful Visual Studio tag builds", async () => {
  ...
  assert.match(pagesWorkflow, /- "!visualstudio-v\*"/);
  ...
  assert.ok(build.includes("scripts/release-version.cjs --stamp"));
  assert.ok(build.indexOf("scripts/release-version.cjs --stamp") < build.indexOf("/t:Rebuild"));
