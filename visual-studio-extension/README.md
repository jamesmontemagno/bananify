...
2. Create an exact release tag, for example **`visualstudio-v0.1.1`** for version **`0.1.1`**, normally after merging the release commit to `main`. Three or four numeric components are supported, without leading zeros or prerelease/build suffixes.
3. CI derives the release version from the tag and stamps `Identity.Version` in `src/Bananify/source.extension.vsixmanifest`, `Version` in `Directory.Build.props`, and the About-box version in `BananifyPackage.cs` in the build workspace. These generated changes are not committed. The downloaded VSIX is checked against the tag again before publishing.
...
Run `node visual-studio-extension/scripts/release-version.cjs` locally to check committed development versions. To reproduce release stamping, set `RELEASE_TAG` to the intended tag and run the script with `--stamp`; this modifies the three local version sources. Use `Verify-Vsix.ps1 -Path <file.vsix> -ReleaseTag <tag>` to validate an actual artifact (Node.js is required for the tag check). Run the release regressions with `node --test visual-studio-extension/tests/release.test.mjs`.
...
The configured publisher ID is `vs-publisher-473885`, and the internal listing name is `Bananify-VisualStudio`, separate from the VS Code product. If updating an existing Visual Studio listing, use its existing internal name instead. Preserve the VSIX identity and use the appropriate next release version for an update.
